                ...
                <div class="cf center">
                   ...
                   <div class="cf center card-marketpair br3 f5-ns f6 w-100 mt1">
                        ...
                        <div class="fl w-100 pt2 tc inline-flex flex-row items-center">
                            ...
                            <span class="w-60 relative orange pt1">
                                ...
                                <small class="pointer" click="buyPriceType = (buyPriceType=='marketprice') ? '' : 'marketprice',sellPriceType = (sellPriceType=='marketprice') ? '' : 'marketprice'">
                                    ...
                                    <span @click="neworder.SellPrice = truncateNumber(market.MiddleBand), updateQtyBaseSell(), sellPriceType = ''" v-if="market.Price < market.MiddleBand" class="db"><small>MB:</small> {{truncateNumber(market.MiddleBand)}}</span>
                                    <span @click="neworder.SellPrice = truncateNumber(market.UpperBand), updateQtyBaseSell(), sellPriceType = ''" v-else class="db" :class="{'black':market.Price < market.UpperBand, 'green':market.Price > market.UpperBand}"><small>UB:</small> {{truncateNumber(market.UpperBand)}}</span>
                                    ...
                                    <span @click="neworder.BuyPrice = truncateNumber(market.MiddleBand), updateQtyBaseBuy(), buyPriceType = ''" v-if="market.Price > market.MiddleBand" class="db"><small>MB:</small> {{truncateNumber(market.MiddleBand)}}</span>
                                    <span @click="neworder.BuyPrice = truncateNumber(market.LowerBand), updateQtyBaseBuy(), buyPriceType = ''" v-else class="db" :class="{'black':market.Price > market.LowerBand, 'red':market.Price < market.LowerBand}"><small>LB:</small> {{truncateNumber(market.LowerBand)}}</span>
                        ...
                        <div class="f7 pt1 flex fl w-100 tl gray">
                            <div class="w-40 ph1 tl pointer" @click="updateBuyPrice(truncateNumber(market.LowPrice)),buyPriceType=''"><small class="i db">(24hr low)</small> {{truncateNumber(market.LowPrice)}} </div>
                            ...
                            <div class="w-40 ph1 tr pointer" @click="updateSellPrice(truncateNumber(market.HighPrice)),sellPriceType=''"> <small class="i db">(24hr high)</small> {{truncateNumber(market.HighPrice)}} </div>
                        </div>
