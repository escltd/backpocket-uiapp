<template>
    <header class="flex w-100 inline-flex items-center bb b--black-10 mw7 center">
        <div class="w-50-l w-40-m w-auto pv3 ph2 inline-flex items-center">
            ...
            <router-link to="/markets" :class="{ 'near-black fade-in': lSwitchmenu, 'near-black': !lSwitchmenu }"
                         class="ml2 pointer mt1 f5 fw6 link pa2 dn dib-ns">
                Markets
            </router-link>

            <router-link to="/orders" :class="{ 'near-black fade-in': lSwitchmenu, 'black-30': !lSwitchmenu }"
                         class="mh2 pointer mt1 f5 fw6 link pa2 dn dib-ns">
                Orders
            </router-link>

            <router-link to="/assets" :class="{ 'near-black fade-in': lSwitchmenu, 'black-30': !lSwitchmenu }"
                         class=" pointer mt1 f5 fw6 link pa2 dn dib-l">
                Assets
            </router-link>

            <router-link to="/markets" :class="{ 'near-black fade-in': lSwitchmenu, 'black-30': !lSwitchmenu }"
            ...
            <span class="fr inline-flex items-center f7">
                <div class="pointer br3 fw6 bg-black-10 near-black ba b--black-10"
                    ...
                    <div class="inline-flex items-center dim">
                        <p class="dib ma0 ph2 pv2 v-mid br3 br--left"> <small><i class="fa"
                                   :class="{ 'fa-caret-down': !showAssets, 'fa-circle light-green': showAssets }" /></small>
                            {{ getDefaultAsset.Symbol }}</p>
                    ...
                    <div class="w-100 relative fl ">
                        <div v-if="showAssets" class="fade-in absolute top-2 bg-background br3 w-100 z-max">
                            <div class="bg-black-10 br3 w-100 h-100 overflow-scroll" style="max-height: 60vh;">
                                ...
                                <div class="w-100 dim pa2 tc bg-near-black near-black bb b--black-05"> zero balances are
                                    below </div>
                                <div id="emptyassets">
                    ...
                    <div class="bg-black-10 h-100 flex flex-column br3 ">
                        ...
                        <div class="w-100 overflow-scroll pointer" style="max-height: 16rem;">
                            <div v-for="(market, index) in getEnabledMarkets()" :key="index"
                                 class="w-100 inline-flex items-center justify-between dim ph2 pv3 bb b--black-05"
                                 @click="$store.dispatch('marketpairs/disableMarketPair', market), showSettings = !showSettings">
                                <p class="dib ma0 pr0 v-mid "> <small>{{ market.BaseAsset }} /
                                        {{ market.QuoteAsset }}</small> </p> <i
                                   class="fr ml1 f7 fa fa-check-square light-green" />
                            </div>
                            <div id="disabledmarket">
                                <div v-for="(market, index) in getDisabledMarkets()" :key="index * index"
                                    ...
                                    <p class="dib ma0 pr0 v-mid "> <small>{{ market.BaseAsset }} /
                                            {{ market.QuoteAsset }}</small></p> <i
                                       class=" ml1 f7 fa fa-square black-10" />
                                </div>
                            </div>
                        </div>
                    </div>
